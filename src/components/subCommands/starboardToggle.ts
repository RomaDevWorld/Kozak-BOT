...
const StarboardToggleSubcommand: SubCommand = {
  ...
  execute: async function (interaction) {
    ...
    await Modules.findOneAndUpdate({ guildId: interaction.guildId }, { 'starboard.status': state }, { upsert: true })

    interaction.reply({ content: state ? t('enabled', { lng }) : t('disabled', { lng }), ephemeral: true })
  },
}

export default StarboardToggleSubcommand
